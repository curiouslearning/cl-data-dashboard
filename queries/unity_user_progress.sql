WITH
  ...
  SELECT
    ...
    CASE
      WHEN event_name = 'GamePlay' AND params.key = 'action' AND params.value.string_value LIKE 'LevelSuccess%' THEN PARSE_DATE('%Y%m%d', event_date)
      ELSE NULL
  END
    AS level_success_date,
    -- Max user level
    CASE
      WHEN event_name = 'GamePlay' AND params.key = 'action' AND params.value.string_value LIKE 'LevelSuccess%' THEN CAST(SUBSTR(params.value.string_value, (STRPOS(params.value.string_value, '_') + 1)) AS INT64)
      ELSE NULL
  END
    AS user_level,
  ...
  SELECT
    ...
    ARRAY_LENGTH( ARRAY(
      SELECT
        level
      FROM
        UNNEST(expected_levels) AS level
      WHERE
        level NOT IN UNNEST(played_levels) ) ) > 0 AS skipped_level
  FROM
    level_check ),
  with_deltas AS (
  SELECT
...
SELECT
  ...
  MIN(CASE
      WHEN a.event_name = 'GamePlay' AND a.params_key = 'action' AND a.params_value_string_value = 'LevelSuccess_25' THEN a.event_date_parsed
  END
    ) AS ra_date,
  DATE_DIFF( MIN(CASE
        WHEN a.event_name = 'GamePlay' AND a.params_key = 'action' AND a.params_value_string_value = 'LevelSuccess_25' THEN a.event_date_parsed
    END
      ), MIN(a.first_open), DAY ) AS days_to_ra,
...
END
  ...
  CASE
    WHEN IFNULL(MAX(a.user_level), 0) >= 1 AND SAFE_DIVIDE(IFNULL(MAX(a.user_level), 0), MAX(a.max_game_level)) * 100 >= 90 THEN 1
    ELSE 0
END
  AS gc_flag,
